    ...
    var methods = {
        ...
        emit: function (event, arg1, arg2__) {
            ...
            if ((caches = this[listenersName]) && (cache = caches[event]) && (ii = cache.length)) {
	            // 之所以要copy一遍，是因为有一些 listener 是 once 一次性的，执行完之后，会自动从数组中删除
	            // 这就导致数组长度被改变了，后面的 i 取不到 listener 而报错了
	            var cc = []
	            for (i = 0; i < ii; i++) cc.push(cache[i])
		        for (i = 0; i < ii; i++) cc[i].apply(this, args);
	            while(cc.length) cc.pop()
	            cc = null
            }
